...
2. **方案 B：开源播放器替换版 ([`沪上插班生视频播放器替换.user.js`](./沪上插班生视频播放器替换.user.js))**  
   使用现代化开源播放器 **Plyr** 接管网页原生播放器，保留原始视频地址；原生 HLS 解析失败时尝试 hls.js 回退。提供全键盘快捷键操控（空格暂停/播放、方向键快进快退与音量调节、M静音、F全屏）、多档倍速、画中画（PiP），自动屏蔽官方冗余浮层与去水印，同样集成智能后台防暂停。
...
- **视频接管与兼容回退**：接管页面现有的 `<video>`；正常播放时沿用原管线。原生 `.m3u8` 出现错误码 4 时尝试用 hls.js 加载原地址，不改写签名参数。
...
实现参考：[HTMLMediaElement.load() 的重置与中止行为](https://developer.mozilla.org/en-US/docs/Web/API/HTMLMediaElement/load)。本地验证：`node --test tests/*.test.cjs`。

#### 方案 B 1.4.0：原生 HLS 解析失败回退

针对 `.m3u8`、错误码 4（例如 `DEMUXER_ERROR_COULD_NOT_PARSE`）的情况，即使视频因错误而处于暂停状态，也会尝试加载固定版本 hls.js 1.6.13，通过 MSE 播放同一地址。每个视频控制器对最近 20 个地址各自动尝试一次回退；原生播放正常、站点已有的 `blob:` 管线和媒体流对象不会触发回退。

- hls.js 通过独立 ES 模块按需加载，不覆盖网站的 `window.Hls`。模块等待上限 15 秒，媒体启动等待上限 45 秒。
- 成功加载后尽可能恢复进度和播放设置；原本已暂停时保留暂停，请点击播放。回退等待期间用户操作会取消自动续播与旧进度恢复。
- 切课或销毁时释放回退实例，过期的异步任务不会接管新视频。
- 网络重试交给 hls.js 的有限加载策略，致命网络错误后停止；媒体元素出错时最多额外恢复一次。
- 查看 `__tbVideoPlayer.recovery.state().hls`：`ready` 表示媒体已可播放；`lastError` 提供错误类型、细节枚举和可用的 HTTP 状态码。诊断不包含完整视频地址、签名或响应正文。

这是一条兼容性回退路径，不能保证修复无效清单、损坏分片、鉴权或服务器错误。hls.js 需要浏览器支持 MSE，且媒体服务器允许跨域读取；模块也可能被页面 CSP 或网络限制拦截。回退失败时仍可能需要刷新或由平台修复。实现依据：[hls.js 1.6.13 官方错误处理说明](https://github.com/video-dev/hls.js/blob/v1.6.13/docs/API.md#fifth-step-error-handling)。

#### 方案 B 1.4.1：切回页面时画面恢复

继续保留后台播放，不因切页主动暂停。通过捕获的真实可见性变化重新挂载帧监控，切回后前 10 秒采用更快的检测：时间轴持续前进，但约 1.5～2.5 秒没有新画面时，在当前缓冲范围内尝试 40ms 微跳；仍未恢复则间隔至少 4 秒尝试 120ms 微跳。正常前台播放的无帧检测阈值仍为 4 秒。

画面恢复使用独立预算，每次切回最多两次；观察到三个推进且接近当前播放时间的帧后恢复预算。此路径也适用于 `blob:` / HLS 视频，不调用 `pause()`、`load()` 或重建 HLS。暂停、拖动、无缓冲时不执行；微跳本身仍可能有极短的音频跳动，不能保证硬件解码异常都能恢复。
...
A：后台渲染调度、解码器或网络异常都可能有关，不能仅凭症状确定原因。方案 A 保留原有的回前台微跳与帧监控。方案 B 1.4.1 会在切回后加快帧检测，确认画面停滞后有限微跳同步，并继续保留后台播放；声音和进度也停止时走上述 12 秒停滞检测。恢复不保证成功，尤其是硬件解码、平台 HLS 请求或鉴权失效时。
